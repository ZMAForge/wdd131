...
function search () {
    ...
    let filteredReceipes = recipes.filter(filterReceipes);

    function filterReceipes(item) {
        return (
    ...
    //TODO: Sort List By Star Rating
	let sortedReceipes = filteredReceipes.sort(sortByRating);
    ...
    //TODO: For each item in list create and append an Article to the main (Reference/Replace HTML Page)
	// (Maybe make a seperate function that takes a list?)
	recipeContainer.innerHTML = "";
	sortedReceipes.forEach(function(recipe) {
		renderRecipe(recipe);
	})
}
...
function recipeTemplate(recipe) {
	return `<article class="recipe-card">
            ...
            <div class="card-content">
                <button disabled>${tagTemplate(recipe.tags)}</button>
                <h3>${recipe.name}</h3>
                <p>${ratingTemplate(recipe.rating)}
